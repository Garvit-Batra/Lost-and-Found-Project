...
mongoose.connect("mongodb+srv://Garvit:"+process.env.PASSWORD+"@cluster0.6cndibr.mongodb.net/LostAndFound",{useNewUrlParser:true});
const connection = "mongodb+srv://Garvit:"+process.env.PASSWORD+"@cluster0.6cndibr.mongodb.net/LostAndFound";
...
app.get("/Dashboard",redirectLogin,function(req,res){
    user.findOne({email:req.session.emailID},function(err,data){
        ...
        const transformedName = name.substring(0,1).toUpperCase() + name.substring(1,name.length).toLowerCase();
...
app.get("/article/:topic",redirectLogin,function(req,res){
    ...
    lost.findOne({_id:post},function(err,result){
        if(result){
            return res.render("Article",{title:result.title,content:result.description,owner:result.username})
        }})
...
app.post("/AddItem",function(req,res){
    ...
    const transformedTitle = Title.substring(0,1).toUpperCase() + Title.substring(1,Title.length).toLowerCase();
    const transformedDesc = Desc.substring(0,1).toUpperCase() + Desc.substring(1,Desc.length).toLowerCase();
